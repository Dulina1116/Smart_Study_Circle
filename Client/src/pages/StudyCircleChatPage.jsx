...
import { ArrowLeft, Download, FileText, MoreVertical, Paperclip, Phone, Search, Send, Smile, Video, Users } from 'lucide-react'
...
const isValidAvatarUrl = (url) => {
  ...
  return url.startsWith('http://') || url.startsWith('https://') || url.startsWith('/uploads/') || url.startsWith('data:') || url.startsWith('/')
}
...
const getAvatarUrl = (url) => {
  ...
  if (url.startsWith('profile-') || url.startsWith('chat/')) relativePath = `/uploads/${url}`;
...
const fileTypeLabel = (mimeType, fileName) => {
  ...
  if (type.includes('word') || name.endsWith('.doc') || name.endsWith('.docx')) return 'DOCX'
  if (type.includes('sheet') || name.endsWith('.xls') || name.endsWith('.xlsx')) return 'XLSX'
  return 'FILE'
}
...
export default function StudyCircleChatPage() {
  ...
  const offlineMembers = members.filter((m) => !onlineUserIds.has(String(m.id)))
    ...
    const load = async () => {
      ...
      try {
        const [circleRes, messageRes] = await Promise.all([
          requestWithTimeout(`${API_BASE}/${circleId}`, { headers: createHeaders() }),
          requestWithTimeout(`${API_BASE}/${circleId}/messages`, { headers: createHeaders() }),
        ])
        ...
        if (err.name === 'AbortError') {
          ...
          setError('Could not connect to chat server. Ensure backend is running.')
        } else {
  ...
  useEffect(() => {
    ...
    const onTyping = ({ circleId: incomingCircleId, userId, displayName, fullName, isTyping }) => {
      if (String(incomingCircleId) !== String(circleId)) return
    ...
    const onRead = ({ circleId: incomingCircleId, readerId }) => {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          String(m.sender?.id || m.senderId) === myId ? { ...m, status: 'read' } : m
        )
      )
    }
  ...
  const uploadFile = async (event) => {
    ...
    try {
      ...
      const res = await requestWithTimeout(`${API_BASE}/${circleId}/messages/upload`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    <div className="h-screen bg-[#0f1115] text-slate-100 flex">
      <div className="flex-1 min-w-0 flex">
        <div className="flex-1 min-w-0 flex flex-col">
          <header className="h-16 bg-[#141922] border-b border-[#222a36] px-3 md:px-5 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className="text-base md:text-lg font-bold text-slate-100 truncate">{circle?.subject || 'Study Circle Chat'}</h1>
                <p className="text-xs text-slate-400 truncate">
                  {circle?.moduleCode ? `${circle.moduleCode} • ${circle.semester} • Year ${circle.year} • ${onlineCount} online` : 'Realtime group chat'}
                </p>
              </div>
            </div>

            <div className="flex items-center gap-1.5">
              <button className="w-9 h-9 rounded-full hover:bg-[#1e2633] text-slate-300 inline-flex items-center justify-center" aria-label="Call">
                <Phone className="w-4 h-4" />
              </button>
              <button className="w-9 h-9 rounded-full hover:bg-[#1e2633] text-slate-300 inline-flex items-center justify-center" aria-label="Video call">
                <Video className="w-4 h-4" />
              </button>
              <button className="w-9 h-9 rounded-full hover:bg-[#1e2633] text-slate-300 inline-flex items-center justify-center" aria-label="Search">
                <Search className="w-4 h-4" />
              </button>
              <button className="w-9 h-9 rounded-full hover:bg-[#1e2633] text-slate-300 inline-flex items-center justify-center" aria-label="Menu">
          ...
          {error ? (
            <div className="mx-4 mt-3 rounded-lg bg-red-900/40 border border-red-500/30 p-3 text-sm text-red-200">{error}</div>
          ) : null}
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex justify-center">
                  <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#26313d] text-slate-300">Today</span>
                </div>

                {messages.map((message) => {
                  const mine = String(message.sender?.id) === myId
                  const hasFile = message.messageType === 'file' && message.fileUrl
                  const senderName = message.sender?.displayName || message.sender?.fullName || 'Student'

                  return (
                    <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[90%] md:max-w-[70%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                        {!mine ? <p className="text-xs font-semibold text-emerald-400 ml-1 mb-1">{senderName}</p> : null}

                        <div className={`rounded-2xl px-3 py-2 shadow-md ${mine ? 'bg-[#1f8f4d] text-white rounded-br-md' : 'bg-[#242a35] text-slate-100 rounded-bl-md border border-[#313846]'}`}>
                          {message?.isForwarded ? (
                            <p className="text-[11px] italic mb-1 text-slate-300">Forwarded</p>
                          ) : null}
                          ...
                          {hasFile ? (
                            <div className="space-y-2">
                              {isImageFile(message.mimeType) ? (
                                ...
                                <div className="rounded-xl border border-white/15 bg-black/20 px-3 py-2 flex items-center justify-between gap-3">
                                  <div className="flex items-center gap-2 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <p className="text-sm font-semibold truncate">{message.fileName || 'Attachment'}</p>
                                      <p className="text-[11px] opacity-80">{fileTypeLabel(message.mimeType, message.fileName)} • {formatFileSize(message.fileSize)}</p>
                                    </div>
                                  </div>
                                  <a
                              ...
                              {message.text ? <p className="text-sm whitespace-pre-wrap break-words">{message.text}</p> : null}
                            </div>
                          ) : (
                            <p className="text-sm whitespace-pre-wrap break-words">{message.text}</p>
                          )}

                          <div className={`mt-1 text-[11px] inline-flex items-center gap-1 w-full justify-end ${mine ? 'text-emerald-100' : 'text-slate-400'}`}>
                            <span>{formatTime(message.createdAt)}</span>
                            {mine ? <span className={message.status === 'read' ? 'text-cyan-200' : 'text-slate-300'}>✓✓</span> : null}
                          </div>
                        </div>
                      </div>
                    </div>
          ...
          <footer className="bg-[#141922] border-t border-[#222a36] px-2 md:px-3 py-2.5">
            <div className="flex items-center gap-2">
              <button type="button" className="w-10 h-10 rounded-full hover:bg-[#1e2633] text-slate-400 inline-flex items-center justify-center" aria-label="Emoji">
            ...
            {typingUsers.size > 0 ? (
              <p className="text-[11px] text-emerald-400 ml-14 mt-1">
                {Array.from(typingUsers.values()).slice(0, 2).join(', ')} {typingUsers.size > 1 ? 'are' : 'is'} typing...
              </p>
            ) : null}
          </footer>
        </div>
        ...
        <aside className="hidden lg:flex lg:w-[350px] border-l border-[#20252e] bg-[#121821] flex-col">
          <div className="h-[92px] px-4 border-b border-[#20252e] flex flex-col justify-center">
            <div className="flex items-center justify-between">
              ...
              <span className="w-7 h-7 rounded-full bg-emerald-500 text-white text-xs font-bold inline-flex items-center justify-center">{members.length}</span>
            </div>
            <p className="text-sm text-slate-400 mt-1">{onlineMembers.length} online • {offlineMembers.length} offline</p>
          </div>

          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div>
              <p className="text-sm font-bold text-emerald-400 mb-2">ONLINE — {onlineMembers.length}</p>
              <div className="space-y-2">
                {onlineMembers.map((member) => {
                  ...
                  return (
                    <div key={member.id} className="rounded-xl bg-[#1b2431] border border-[#2a3445] px-3 py-2.5 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full overflow-hidden bg-emerald-500 flex-shrink-0 flex items-center justify-center text-base font-bold text-white">
                        {isValidAvatarUrl(member.avatar) && !failedAvatars.has(String(member.id)) ? (
                          <img
                            src={getAvatarUrl(member.avatar)}
                            alt={member.displayName || member.fullName || 'Member'}
                            className="w-full h-full object-cover"
                            onError={() => handleAvatarError(member.id)}
                          />
                        ) : isValidAvatarUrl(member.profilePicture) && !failedAvatars.has(String(member.id)) ? (
                          <img
                          ...
                          <span>{(member.displayName || member.fullName || 'S').charAt(0).toUpperCase()}</span>
                        )}
                      </div>

                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-slate-100 truncate">{member.displayName || member.fullName || member.email}{mine ? ' (You)' : ''}</p>
                        <p className="text-xs text-slate-400 truncate">{role}</p>
                      </div>

                      {role === 'Moderator' || role === 'Co-Moderator' ? (
                        <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-[#1f8f4d] text-white">MOD</span>
                      ) : null}
                    </div>
                  )
                })}
            ...
            <div>
              <p className="text-sm font-bold text-slate-400 mb-2">OFFLINE — {offlineMembers.length}</p>
              <div className="space-y-2">
                {offlineMembers.length === 0 ? (
                  ...
                  offlineMembers.map((member) => {
                    ...
                    return (
                      <div key={member.id} className="rounded-xl bg-[#161d28] border border-[#242e3d] px-3 py-2.5 flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-slate-200 truncate">{member.displayName || member.fullName || member.email}{mine ? ' (You)' : ''}</p>
