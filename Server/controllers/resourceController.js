...
export const getResources = async (req, res) => {
  try {
    const resources = await Resource.find({ uploader: req.user._id }).sort('-createdAt');
    res.json(resources);
  } catch (error) {
...
export const getTopResources = async (req, res) => {
  try {
    const resources = await Resource.find({ uploader: req.user._id }).sort('-createdAt').limit(5);
    res.json(resources);
  } catch (error) {
    res.status(500).json({ message: 'Error fetching resources', error: error.message });
  }
...
export const deleteResource = async (req, res) => {
  try {
    const resource = await Resource.findOneAndDelete({ _id: req.params.id, uploader: req.user._id });
    if (!resource) return res.status(404).json({ message: 'Resource not found' });
