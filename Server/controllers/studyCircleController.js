...
const mapCircle = (circle, currentUserId) => {
  ...
  const moderatorIds = (circle.coModerators || []).map((m) => String(m._id || m))
  ...
  return {
    ...
    creator: circle.creator?._id ? sanitizeUser(circle.creator) : { id: circle.creator },
    coModerators: (circle.coModerators || []).map((cm) => (cm?._id ? sanitizeUser(cm) : { id: cm })),
    members: (circle.members || []).map((m) => (m?._id ? sanitizeUser(m) : { id: m })),
    memberCount: circle.memberCount || (circle.members ? circle.members.length : 0),
    pendingJoinRequests,
    myMembership: {
...
export const createStudyCircle = async (req, res) => {
  try {
    const { subject, moduleCode, semester, year, visibility, description } = req.body
    const parsedYear = Number(year)

    if (!subject || !moduleCode || !semester || !year) {
      return res.status(400).json({ message: 'Subject, module code, semester and year are required.' })
    }
    ...
    const circle = await StudyCircle.create({
      ...
      description: description ? String(description).trim().substring(0, 250) : '',
      inviteCode,
    ...
    const populated = await StudyCircle.findById(circle._id)
      ...
      .populate('coModerators', 'fullName displayName email avatar profilePicture')
      .populate('members', 'fullName displayName email avatar profilePicture')
      .populate('joinRequests.user', 'fullName displayName email avatar profilePicture')
    ...
    return res.status(500).json({ message: 'Server error creating study circle.' })
  }
}
...
export const getMyStudyCircles = async (req, res) => {
  try {
    const circlesRaw = await StudyCircle.find({
      ...
      .select('subject moduleCode semester year visibility inviteCode creator coModerators members joinRequests createdAt updatedAt')
      .sort({ updatedAt: -1 })
      .lean()
      .populate('creator', 'fullName displayName email avatar profilePicture')
      .populate('coModerators', 'fullName displayName email avatar profilePicture')
...
export const getDiscoverCircles = async (req, res) => {
  try {
    ...
    const circles = circlesRaw.map((c) => ({
      ...
      creator: c.creator?._id ? sanitizeUser(c.creator) : { id: c.creator?._id || c.creator },
      coModerators: [],
      members: [],
    ...
    return res.status(500).json({ message: 'Server error loading discover circles.' })
  }
}
...
export const requestJoinCircle = async (req, res) => {
  try {
    ...
    const existingRequest = circle.joinRequests.find((jr) => String(jr.user) === meId && jr.status === 'pending')
    if (existingRequest) {
...
export const joinCircleByInviteCode = async (req, res) => {
  try {
    ...
    if (!circle) {
      ...
      if (publicCircle) {
        return res.status(400).json({ message: 'This is a public circle. Join directly from the Discover Public Circles section.' })
      }
    ...
    const existingPending = circle.joinRequests.find((jr) => String(jr.user) === meId && jr.status === 'pending')
    if (existingPending) {
      return res.status(400).json({ message: 'You already have a pending join request for this circle.' })
    }

    circle.joinRequests.push({ user: req.user._id, status: 'pending' })
    await circle.save()
    return res.json({ message: 'Invite code accepted. Join request sent to moderators for approval.' })
  } catch (err) {
...
export const respondToJoinRequest = async (req, res) => {
  try {
    ...
    if (!isModerator(circle, req.user._id)) {
      return res.status(403).json({ message: 'Only moderators can manage requests.' })
    }
    ...
    return res.status(500).json({ message: 'Server error updating join request.' })
  }
}
...
export const assignCoModerator = async (req, res) => {
  try {
    ...
    if (String(circle.creator) !== String(req.user._id)) {
      return res.status(403).json({ message: 'Only the circle creator can assign co-moderators.' })
    }

    if ((circle.coModerators || []).length >= 2) {
      return res.status(400).json({ message: 'Maximum 2 co-moderators can be assigned.' })
    }

    let targetUser = null

    if (userId) {
      targetUser = await User.findById(userId).select('_id email role isVerified')
    } else {
      const normalizedEmail = String(email).trim().toLowerCase()
      targetUser = await User.findOne({ email: normalizedEmail }).select('_id email role isVerified')
    }

    if (!targetUser) {
      return res.status(404).json({ message: 'Student not found for the provided email.' })
    }

    if (targetUser.role !== 'student') {
      return res.status(400).json({ message: 'Only student accounts can be assigned as co-moderators.' })
    }

    if (!targetUser.isVerified) {
      return res.status(400).json({ message: 'Student account is not verified yet.' })
    }

    const targetUserId = String(targetUser._id)

    if (String(circle.creator) === targetUserId) {
      return res.status(400).json({ message: 'Creator cannot be assigned as co-moderator.' })
    }

    if (circle.coModerators.some((m) => String(m) === targetUserId)) {
      return res.status(400).json({ message: 'This student is already a co-moderator.' })
    }
    ...
    circle.joinRequests = (circle.joinRequests || []).filter((jr) => String(jr.user) !== targetUserId)

    circle.coModerators.push(targetUser._id)
    await circle.save()

    return res.json({ message: 'Co-moderator assigned successfully. Student joined the circle automatically.' })
  } catch (err) {
    console.error('Assign Co-Moderator Error:', err.message)
    return res.status(500).json({ message: 'Server error assigning co-moderator.' })
  }
}
...
export const removeCoModerator = async (req, res) => {
  try {
    ...
    if (String(circle.creator) !== String(req.user._id)) {
      return res.status(403).json({ message: 'Only the circle creator can remove co-moderators.' })
    }

    circle.coModerators = circle.coModerators.filter((cm) => String(cm) !== String(userId))
    await circle.save()

    return res.json({ message: 'Co-moderator removed successfully.' })
  } catch (err) {
    console.error('Remove Co-Moderator Error:', err.message)
    return res.status(500).json({ message: 'Server error removing co-moderator.' })
  }
}
...
export const leaveCircle = async (req, res) => {
  try {
    ...
    if (String(circle.creator) === meId) {
      return res.status(400).json({ message: 'Creator cannot leave the circle. Transfer ownership first (not implemented).' })
    }

    circle.members = circle.members.filter((m) => String(m) !== meId)
    circle.coModerators = circle.coModerators.filter((m) => String(m) !== meId)
    circle.joinRequests = circle.joinRequests.filter((jr) => String(jr.user) !== meId)
...
export const reportCircle = async (req, res) => {
  try {
    ...
    const hasAlreadyReported = circle.reports.some((r) => String(r.user) === String(req.user._id))
    if (hasAlreadyReported) {
      return res.status(400).json({ message: 'You have already reported this circle.' })
    }
    ...
    await circle.save()
    return res.json({ message: 'Circle reported successfully. Our team will review it.' })
  } catch (err) {
...
export const deleteStudyCircle = async (req, res) => {
  try {
    ...
    if (String(circle.creator) !== String(req.user._id)) {
      return res.status(403).json({ message: 'Only the study circle owner can delete this circle.' })
    }
    ...
    return res.status(500).json({ message: 'Server error deleting study circle.' })
  }
}
...
export const updateStudyCircle = async (req, res) => {
  try {
    ...
    const isCoModerator = circle.coModerators.some((mod) => String(mod._id) === userId)

    if (!isCreator && !isCoModerator) {
      return res.status(403).json({ message: 'Only the circle owner and co-moderators can update this circle.' })
    }
    ...
    if (visibility && ['public', 'private'].includes(visibility)) circle.visibility = visibility
    if (description !== undefined) circle.description = String(description).trim().substring(0, 250)
    ...
    return res.status(500).json({ message: 'Server error updating study circle.' })
  }
}
...
export const getStudyCircleById = async (req, res) => {
  try {
    ...
    if (!isMember && circle.visibility === 'private') {
      return res.status(403).json({ message: 'This private circle can only be viewed by members.' })
    }
...
export const getCircleMessages = async (req, res) => {
  try {
    const { circleId } = req.params
    const circle = await StudyCircle.findById(circleId).select('members visibility isActive')
    ...
    if (!isMemberOfCircle(circle, req.user._id)) {
      return res.status(403).json({ message: 'Only circle members can access chat.' })
    }
...
export const sendCircleMessage = async (req, res) => {
  try {
    ...
    if (!isMemberOfCircle(circle, req.user._id)) {
      return res.status(403).json({ message: 'Only circle members can send messages.' })
    }
...
export const sendCircleFileMessage = async (req, res) => {
  try {
    ...
    if (!isMemberOfCircle(circle, req.user._id)) {
      return res.status(403).json({ message: 'Only circle members can upload files.' })
    }
    ...
    return res.status(500).json({ message: 'Server error uploading file message.' })
  }
}
