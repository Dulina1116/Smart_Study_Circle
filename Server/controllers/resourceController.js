...
export const getAllResources = async (req, res) => {
  try {
    const { category, type, search, sort = '-createdAt', limit = 20, page = 1 } = req.query
    ...
    const resources = await Resource.find(filter)
      .populate('uploadedBy', 'fullName displayName email avatar profilePicture')
      .sort(sort)
      .skip(skip)
      .limit(Number(limit))
...
export const getFeaturedResources = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Server error fetching featured resources.' })
  }
}
...
export const createResource = async (req, res) => {
  try {
    const { title, description, category, type, externalLink, isLecturerRecommended, duration } = req.body
    ...
    const populated = await resource.populate('uploadedBy', 'fullName displayName email avatar profilePicture')
...
export const updateResource = async (req, res) => {
  try {
    ...
    if (description !== undefined) resource.description = description ? String(description).trim() : ''
    if (category) resource.category = category
    if (type) resource.type = type
    if (externalLink !== undefined) resource.externalLink = externalLink ? String(externalLink).trim() : null
    if (duration !== undefined) resource.duration = duration ? String(duration).trim() : null
    if (isLecturerRecommended !== undefined) resource.isLecturerRecommended = isLecturerRecommended
...
export const trackDownload = async (req, res) => {
  try {
    ...
    return res.json({ message: 'Download tracked.', downloads: resource.downloads })
  } catch (err) {
...
export const getRecentResources = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Server error fetching recent resources.' })
  }
}
