...
export default function StudentCirclesManager({
  user,
  myCircles = [],
  discoverCircles = [],
  refreshCircles,
  ...
  const assignCoModerator = async (circleId) => {
    const emailToAssign = String(coModeratorEmailByCircle[circleId] || "")
      .trim()
      .toLowerCase();
    if (!emailToAssign) {
  ...
  const removeCoModerator = async (circleId, memberId) => {
    await performAction(`remove-mod-${memberId}`, async () => {
      const res = await fetch(
        `${API_BASE}/${circleId}/co-moderators/${memberId}`,
        {
          method: "DELETE",
          headers: createHeaders(),
        },
  ...
  const deleteCircle = async (circleId) => {
    const confirmed = window.confirm(
      "Are you sure you want to delete this study circle? This action cannot be undone.",
    );
    if (!confirmed) return;
              ...
              >
                ...
                {circle.visibility === "private" ? (
                  ...
                  <div className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700">
                    This is a public circle. Other students can join directly
                    from Discover.
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          setCoModeratorEmailByCircle((prev) => ({
                            ...prev,
                        ...
                        {circle.coModerators.map((mod) => (
                          ...
                          >
                            <span className="inline-flex items-center">
                              ...
                              {mod.displayName || mod.fullName || mod.email}
                            </span>
                            <button
                              type="button"
                              onClick={() =>
              ...
              <div className="space-y-5">
                <div className="rounded-xl border border-gray-100 p-4 bg-gray-50">
                  <h4 className="text-lg font-bold text-gray-900">
                    {detailCircle.subject}
                  </h4>
                  <p className="text-sm text-gray-600 mt-1">
                    {detailCircle.moduleCode} • {detailCircle.semester} • Year{" "}
                    {detailCircle.year}
                  </p>
                  <p className="text-xs font-semibold mt-2 text-gray-500 uppercase tracking-wide">
                    Visibility: {detailCircle.visibility}
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-gray-100 p-4">
                  ...
                  <p className="text-sm text-gray-700">
                    {detailCircle.creator?.displayName ||
                      detailCircle.creator?.fullName ||
                      "Owner"}
                  </p>
                  <p className="text-xs text-gray-500">
                    {detailCircle.creator?.email || "Email not available"}
                  </p>
                </div>

                <div className="rounded-xl border border-gray-100 p-4">
                  <h5 className="text-sm font-bold text-gray-800 mb-2">
                    Co-Moderators ({detailCircle.coModerators?.length || 0})
                  </h5>
                      ...
                      {(detailCircle.coModerators || []).map((member) => (
                        ...
                        >
                          <p className="text-sm font-semibold text-blue-800">
                            {member.displayName || member.fullName || "Student"}
                          </p>
